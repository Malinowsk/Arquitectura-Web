...
public interface ScooterRepository extends JpaRepository<Scooter,Long> {
    ...
    @Query("SELECT s.id,s.model,s.kmsTraveled, CASE WHEN :withPause = true THEN s.totalUsageTime ELSE (s.totalUsageTime-s.pausedTime) END FROM Scooter s ORDER BY 3 DESC")
