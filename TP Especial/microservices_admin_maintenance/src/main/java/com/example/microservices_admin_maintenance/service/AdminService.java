...
public class AdminService {
    ...
    @Transactional
    public DTOResponseStation assignScooterToStation(Long station_id, DTORequestScooter scooterDTO,HttpHeaders headers) {
        if(checkPermissions(headers).is2xxSuccessful()){
            ...
            return this.restTemplate.exchange(station_microservice_uri, HttpMethod.PUT, requestEntity, DTOResponseStation.class).getBody();
        }
    ...
    @Transactional
    public DTOResponseCharged amountEarnedInTimePeriod(int year, int starting_month, int ending_month,HttpHeaders headers) {
        if(checkPermissions(headers).is2xxSuccessful()){
            ...
            return restTemplate.getForObject(scooter_microservice_uri, DTOResponseCharged.class);
        }
        else throw new NotFoundException("error 500");
    }

    @Transactional
    public DTORespondeStatusQualityScooter quantityOfScootersInOperation(HttpHeaders headers) {
        if(checkPermissions(headers).is2xxSuccessful()){
            String scooter_microservice_uri = "http://localhost:8003/api/monopatines/operacion-vs-mantenimiento";
            return restTemplate.getForObject(scooter_microservice_uri, DTORespondeStatusQualityScooter.class);
        }
        else throw new NotFoundException("error 500");
    }
